...
# Calibration runs
cal_root <- here::here("data/outputs/Best_Replicates4")
...
sim_obs_dths_race %>% 
  ggplot(aes(x = Race_Eth, y = Dr_Mean, fill = Val)) +
  theme_classic() +
  theme(axis.text = element_text(size = 14),
        axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title = element_text(size = 16)) +
...
case_race_cal <- bind_rows(lapply(1:ncal_sims, function(i){
  sim <- get_cal_sim(i)
  sim_case_race <- sim$linelist_tests[test_pos == 1 & Date <= as.Date("2020-12-01"), .N, by = race]
  sim_case_race[, sim := i]
  
  rm(sim) ; gc()

  return(sim_case_race)
}))

case_race_sum <- case_race_cal %>% 
  group_by(race) %>% 
...
case_race_sum %>% 
  ggplot(aes(x = Race_Eth, y = C_mean, fill = Val)) +
  theme_classic() +
  theme(axis.text = element_text(size = 14),
        axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title = element_text(size = 16)) +
