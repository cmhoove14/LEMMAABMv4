...
vaxexxA <- readRDS("data/outputs/ABMv4_Dec_Start__bta0.25_vaxvaxessA_2021-02-01.rds")
  nsims <- length(vax65p)
...
vax65p_cases <- rbindlist(lapply(1:nsims, FUN = function(s){
  df <- as.data.frame(vax65p[[s]][["linelist_tests"]]) %>% 
    mutate(iter = s,
           vax = "65pS")
...
vaxessS_cases <- rbindlist(lapply(1:nsims, FUN = function(s){
  df <- as.data.frame(vaxessS[[s]][["linelist_tests"]]) %>% 
    mutate(iter = s,
           vax = "essS")
...
vaxessA_cases <- rbindlist(lapply(1:nsims, FUN = function(s){
  df <- as.data.frame(vaxessA[[s]][["linelist_tests"]]) %>% 
    mutate(ndays = row_number(),
           Date = as.Date(ref_date+ndays),
           iter = s,
           vax = "essA")
