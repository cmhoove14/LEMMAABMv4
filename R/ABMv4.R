# ---------------------------------------------------------
#   COVID ABM v4 new synthetic agents database to include race and investigate disparities
...
#' @title Quarantine duration 
#' 
#' @description Function to generate random number of days of quarantine
#' 
#' @param n_agents number of agents to generate probabilities for
#' 
#' @return vector of length n_agents with randomly generated duration of quarantine following symptom onset/contact/positive test
#' @export
...
#' @title Duration of COVID-like symptoms caused by non-SARS-CoV2 infection
#' 
#' @description Function to generate duration of COVID-like symptoms caused by non-SARS-CoV2 infection
#' 
#' @param n_agents number of agents to draw for
#' 
#' @return vector of length `n_agents`
#' @export

other_symp_dur_fx <- function(n_agents){
  rgamma(n_agents, 3, 1)
}
...
#' @param bta_scl school transmission probability multiplier
#' @param bta_work work transmission probability multiplier
#' @param bta_other other transmission probability multiplier
...
#' @param cbg_cdf_list list of matrices of dim cbgs x cbgs in which entries represent probabilities of moving from CBG i to CBG j. Used in `GetCBGVisit`/`GetCBGVisit_cpp` function
#' @param cbg_ids vector relating row numbers of `cbg_cdf_list` to actual cbg codes
...
#' @param other_ill logical of whether to include circulation of other illness that causes COVID-like symptoms
#' @param init_other initial number of individuals with other pathogens that cause COVID-like illness
#' @param other_bta Relative transmission rate of "other" pathogens that cause covid-like symptoms and are transmitted across the same networks
#' @param other_symp_dur_fx function to generate duration of symptoms for non-COVID illness
...
#' @param adapt_site_geo geography to base adaptive sites on. Can be `cbg`, `ct`, `nbhd`, or `zip`
...
covid_abm_v4 <- function(bta_fx, bta_hh, bta_scl, bta_work, bta_other, E0, Ip0, Ia0, Im0, Imh0, Ih0, R0, D0,                  
                         agents_dt, cbg_cdf_list, cbg_ids, stay_home_dt, visitors, visitors_list, visitor_mult_testing, visitor_mult_sfgrph,
                         ...
                         other_ill, init_other, other_bta, other_symp_dur_fx,
                         testing, test_start, test_delay_fx, tests_pp_fx, tests_pub, tests_wknd, test_probs_pub_fx, test_probs_pvt_fx, 
                         symp_mult, cont_mult, res_mult, nosymp_state_mult, symp_state_mult, hosp_mult, test.red, 
...
# Initial infection allocated among non-children
  init.Es   <- sample(agents[!age %in% c(5,15), id], e.seed)   
  init.Ips  <- sample(agents[!age %in% c(5,15), id], ip.seed)   
  init.Ias  <- sample(agents[!age %in% c(5,15), id], ia.seed)   
  init.Ims  <- sample(agents[!age %in% c(5,15), id], im.seed)   
  init.Imhs <- sample(agents[!age %in% c(5,15), id], imh.seed)   
  init.Ihs  <- sample(agents[!age %in% c(5,15), id], ih.seed)   
  init.Ds   <- sample(agents[!age %in% c(5,15), id], d.seed)   
  init.Rs   <- sample(agents[!age %in% c(5,15), id], r.seed)   
...
#Keep track of extra metrics if store_extra = TRUE  
  ...
  if(store_extra & other_ill){
    other_ill_i <- numeric(length = (t.tot/dt))
    other_ill_i[1] = 0
  }
  
  
# Determine adaptive testing days
  if(class(adapt_start) == "Date"){
...
# Initialize "other" illness
  if(other_ill){
    agents[, other_ill := 0]
    init.others <- sample(agents[state == "S", id], init_other)  
    agents[id %in% init.others, other_ill := 1]
    agents[other_ill == 1, dur_other_symptoms:=other_symp_dur_fx(.N)]
    agents[, other_ill_immune := 0] 
  }
...
# Advance transition times, time since infection,time since symptoms started, other illness symptoms, quarantine duration, and test notification times
  ...
  if(other_ill){
    agents[other_ill == 1, t_symptoms:=t_symptoms+dt]
    agents[other_ill == 1 & t_symptoms > dur_other_symptoms, other_ill_immune:=1]
    agents[other_ill == 1 & t_symptoms > dur_other_symptoms, c("other_ill", "dur_other_symptoms"):=0]
  }
    ...
    # Determine agents who are eligible for each type of testing and get their individual testing probability
      ...
      agents[id %in% eligible_ids,
             test_prob:=test_probs_pub_fx(income_bracket, race, essential, t_symptoms,  
                                          state, t_since_contact, res_inf, residence_type, 
                                          adapt_site, adapt_site_mult, n_tests_pub,
                                          symp_mult, cont_mult, res_mult, nosymp_state_mult, symp_state_mult, hosp_mult)]
      
      agents[id %in% pvt_eligible_ids,
             test_prob:=test_probs_pvt_fx(income_bracket, race, essential, t_symptoms,  
                                          state, t_since_contact, res_inf, residence_type, n_tests_pvt, 
                                          symp_mult, cont_mult, res_mult, nosymp_state_mult, symp_state_mult, hosp_mult)]
      ...
      test_reports[[as.numeric(date_now-t0)]] <- agents[id %in% c(pub_test_ids, pvt_test_ids),
                                                        c("id", "age", "race", "state", "nextstate",
                                                          ...
                                                          "residence", "residence_type", "income",
                                                          "work", "school", "cbg", "ct", "zip", "nbhd", "lat", "lon",
...
# Determine locations ---------------
  ...
  home.today[,cbg:=as.numeric(origin_census_block_group)]
  home.today[,pct_home:=pct_home^(1/4)] # Correct stay at home all day percentage for four possible time steps
  home.today[,c("Date", "origin_census_block_group"):=NULL]
  home.mean <- mean(home.today$pct_home)
  agents <- agents[home.today, on = "cbg"] # merge to get stay home pct by cbg
  ...
  # Find locations of those not deceased, in the hospital, in group quarters, or quarantining  
    ...
    agents[q_duration > 0 | residence_type %in% c("P", "N"), location:=residence]
    agents[state %in% c("S", "E", "Ip", "Ia", "Im", "Imh", "R") & q_duration == 0 & residence_type == "H", 
           mobile:=1]

    # Agents that are in school
    agents[school > 0 & work < 0 & mobile == 1, 
           location:=sac_location( 
             state, scl.closed, SiP.active, pct_home,
    ...
    agents[work < 0 & school < 0 & mobile == 1,
    ...
    cbg_movers <- fmatch(agents[mobile == 1 & location == cbg, location],cbg_ids) # Get cbg indices of movers
    cbg_randnum <- dqrng::dqrunif(length(cbg_movers)) # Random numbers that interact with cdfs
    cbg_moves <- cbg_ids[GetCBGVisit_cpp(cbg_movers, cbg_randnum, cbg_cdf_list[[date_num]])]
    
    agents[mobile == 1 & location == cbg, 
           location:=cbg_moves]
    
    # Smaller sub-locations (offices and classrooms) for agents in workplaces or schools
    agents[mobile == 1 & location == school,
           small_location:=class_id]
    
    agents[mobile == 1 & location == work,
           small_location:=office_id]

  if(verbose){
    cat("Locations resolved\n")
    cat(nrow(agents[location==residence,])/nrow(agents)*100,"% agents at home\n\n")
  }
...
# Determine who's transmitting and number of individuals by location  
  ...
  agents[!is.na(small_location), n_present_small:=.N, by = small_location]
...
# Determine FOI in locations where someone is transmitting
  ...
  agents[infector == 1, 
         trans_prob := beta_today*bta_other*(1-mask.red*wear.mask)*(1-test.red*tested)]

  agents[infector == 1 & location == residence, 
  ...
  agents[infector == 1 & location == school, 
         trans_prob := beta_today*bta_scl*(1-mask.red*wear.mask)*(1-test.red*tested)]
...
# For agents sharing small location, overwrite lower FOI with small location FOI 
  agents[!is.na(small_location), FOI:=sum(trans_prob*infector/n_present_small, na.rm = T), by = small_location]
...
# Transmission of other covid-like illness    
  if(other_ill){
    agents[other_ill == 1, 
           trans_prob_other := beta_today*bta_other*other_bta*(1-mask.red*wear.mask)*(1-test.red*tested)]
  
    agents[other_ill == 1 & location == residence, 
           trans_prob_other := beta_today*bta_hh*other_bta*(1-mask.red*wear.mask*tested)*q_bta_red*(1-test.red*tested)] # Assume no mask wearing at home unless tested positive, reduction in transmission if quarantining based on income
    
    agents[other_ill == 1 & location == work, 
           trans_prob_other := beta_today*bta_work*other_bta*(1-mask.red*wear.mask)*(1-test.red*tested)]
    
    agents[other_ill == 1 & location == school, 
           trans_prob_other := beta_today*bta_scl*other_bta*(1-mask.red*wear.mask)*(1-test.red*tested)]
    
    agents[, FOI_other:=sum(trans_prob_other*other_ill/n_present, na.rm = T), by = location]
    agents[!is.na(small_location), FOI_other:=sum(trans_prob_other*other_ill/n_present_small, na.rm = T), by = small_location]
    
  # Generate other infection
      agents[FOI_other > 0 & other_ill == 0 & other_ill_immune == 0, other_ill_infect:=foi_infect(FOI_other)]
      agents[other_ill_infect == 1, other_ill:=1]
      agents[other_ill_infect == 1, dur_other_symptoms:=other_symp_dur_fx(.N)]
  }  
...
# Quarantine probabilities
  ...
  # Determine known residential infections
    ...
    agents[, res_inf:=sum(res_infector, na.rm = T), by = residence] 
  ...
  if(store_extra){
    stay_home[t] <- nrow(agents[location==residence,])/nrow(agents)*100
  ...
  if(visitors){
    agents <- na.omit(agents, "residence") 
  }  

  
  # Reset infection & location columns and remove temp quarantine objects
    agents[, c("location", "small_location", "mobile", "infector", "res_infector",
               "contact_prob", "contact", "n_present", "n_present_small", "wear.mask",
